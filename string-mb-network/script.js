const STRING_MB_FILES = [
  "../stringMB_interactions_short.tsv",
  "../stringMB_protein_annotations.tsv",
  "../stringMB_network_coordinates.tsv",
];
...
const inferCategory = (annotation = "") => {
  ...
  if (/(kinase|receptor|signal|phosph|growth factor|transcription|chromatin|remodeling)/.test(text)) {
    return "Sinalizacao";
  }

  if (/(adhesion|junction|cytoskeletal|armadillo|epithelial|coiled-coil|scaffold|actin)/.test(text)) {
    return "Adesao / estrutura";
  }

  if (/(transport|transporter|membrane|mitochond|channel|atp-binding|metabolism|splicing|rna|tRNA)/.test(text)) {
...
const loadDatasets = async () => {
  const responses = await Promise.all(
    STRING_MB_FILES.map(async (path) => {
...
const buildNetwork = (classifiedFiles) => {
  ...
  if (!edgeFile) {
    throw new Error("Nenhum arquivo de interacoes foi detectado entre os arquivos stringMB*.");
  }
...
const exportImage = () => {
  ...
  link.href = image;
  link.download = "rede-mb-string.png";
  link.click();
};
...
const initialize = async () => {
  try {
    ...
    elements.scoreFilter.value = state.minScore.toFixed(2);
    elements.datasetSummary.textContent = `${state.nodes.length} proteinas e ${state.edges.length} interacoes integradas de ${filesUsed.length} arquivo(s) stringMB* detectados automaticamente no projeto.`;
