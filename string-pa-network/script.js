const STRING_PA_FILES = [
  "../stringPA_interactions_short.tsv",
  "../stringPA_protein_annotations.tsv",
  "../stringPA_network_coordinates.tsv",
];
...
const inferCategory = (annotation = "") => {
  ...
  if (/(transport|transporter|membrane|mitochond|channel|atp-binding|acyl-coa|metabolism|mitochondrial)/.test(text)) {
...
const loadDatasets = async () => {
  const responses = await Promise.all(
    STRING_PA_FILES.map(async (path) => {
...
const buildNetwork = (classifiedFiles) => {
  ...
  if (!edgeFile) {
    throw new Error("Nenhum arquivo de interacoes foi detectado entre os arquivos stringPA*.");
  }
...
const exportImage = () => {
  ...
  link.href = image;
  link.download = "rede-pa-string.png";
  link.click();
};
...
const initialize = async () => {
  try {
    ...
    elements.scoreFilter.value = state.minScore.toFixed(2);
    elements.datasetSummary.textContent = `${state.nodes.length} proteinas e ${state.edges.length} interacoes integradas de ${filesUsed.length} arquivo(s) stringPA* detectados automaticamente no projeto.`;
